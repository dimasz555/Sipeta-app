...
@section('content')
    <main id="main" class="main">
        ...
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <x-primary-button class="w-full flex justify-center items-center gap-2" style="width: 200px;"
                    title="Tambah Pembelian" data-bs-toggle="modal" data-bs-target="#addPembelian">
            ...
            <div class="table-responsive">
                <table id="tablePembelian" class="table table-striped table-bordered dt-responsive nowrap mx-auto"
                    ...
                    <tbody>
                        @forelse ($pembelian as $bk)
                            <tr>
                                ...
                                <td>{{ $bk->tgl_lunas ? \Carbon\Carbon::parse($bk->tgl_lunas)->translatedFormat('j F Y') : '-' }}
                                </td>
                                <td>{{ $bk->user->name . ' (' . $bk->user->phone . ')' }}</td>
                                <td>{{ $bk->boking->project->name }}</td>
                                <td>{{ $bk->boking->no_blok }}</td>
                                <td>{{ 'Rp ' . number_format($bk->harga, 0, ',', '.') }}</td>
                                <td>
                                    @if ($bk->last_payment_date)
                                        {{ $bk->last_payment_date->translatedFormat('j F Y H:i') }}
                                        <br>
                                        <small class="text-muted">{{ $bk->last_payment_date->diffForHumans() }}</small>
                                    @else
                                ...
                                <td>
                                    <x-button-action style="background-color: #BC55C3;" title="Detail Pembelian">
                                        <a href="{{ route('pembelian.detail', $bk->encrypted_id) }}" class="text-white"><i
                                                class="bi bi-eye"></i></a>
                                    </x-button-action>
                                    @if ($bk->pjb)
                                        <x-button-action style="background-color: #BC55C3;" title="Lihat PJB"
                                            class="text-white">
                                            <a href="{{ asset('storage/' . $bk->pjb) }}" target="_blank"
                                                class="text-white">
                                    ...
                                    <x-button-action style="background-color: #BC55C3;" data-bs-toggle="modal"
                                        data-bs-target="#editPembelianModal" data-id="{{ $bk->id }}"
                                        data-user-id="{{ $bk->user->id }}" data-user-name="{{ $bk->user->name }}"
                                        data-boking-id="{{ $bk->boking->id }}" data-harga="{{ $bk->harga }}"
                                        data-dp="{{ $bk->dp }}"
                                        data-jumlah-bulan-cicilan="{{ $bk->jumlah_bulan_cicilan }}"
                                        data-harga-cicilan-perbulan="{{ $bk->harga_cicilan_perbulan }}"
                                        data-pjb="{{ $bk->pjb }}" title="Edit Data Pembelian">
                                        <i class="bi bi-pencil text-white"></i>
                                    </x-button-action>
                                    @if ($bk->status !== 'batal')
                                        <x-button-action style="background-color: #E33437;" data-bs-toggle="modal"
                                            data-bs-target="#pembatalanModal" data-id="{{ $bk->id }}"
                    ...
                    <form id="formTambahPembelian" method="POST" action="{{ route('tambah.pembelian') }}"
                        ...
                        <div class="modal-body px-sm-5 mx-50 pb-5">
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <select id="boking_id" name="boking_id" class="form-select" required>
                                    ...
                                    @foreach ($boking as $boking)
                                        <option value="{{ $boking->id }}"> {{ $boking->project->name }} -
                                            {{ $boking->no_blok }} - {{ $boking->user->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-12 mb-3">
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <input type="number" class="form-control" id="jumlah_bulan_cicilan"
                                    name="jumlah_bulan_cicilan" required>
                            </div>
                            <div class="col-12 mb-3">
                                <label for="harga_cicilan_perbulan" class="form-label">Harga Cicilan per Bulan</label>
                                <input type="text" class="form-control" id="harga_cicilan_perbulan"
                                    name="harga_cicilan_perbulan" required oninput="formatRupiah(this)">
                            </div>
                            <div class="col-12 mb-3">
                                <label for="pjb_file" class="form-label">PJB</label>
                                <input type="file" class="form-control" id="pjb_file" name="pjb"
                                    accept=".pdf">
                            </div>
        ...
        <div class="modal fade" id="editPembelianModal" tabindex="-1" aria-labelledby="editPembelianModalLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <form id="formEditPembelian" method="POST" action="{{ route('edit.pembelian') }}"
                        ...
                        <div class="modal-body px-sm-5 mx-50 pb-5">
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <select id="edit_boking_id" name="boking_id" class="form-select" required>
                                    ...
                                    @foreach ($pembelian as $pb)
                                        <option value="{{ $pb->boking->id }}"> {{ $pb->boking->project->name }} -
                                            {{ $pb->boking->no_blok }} - {{ $pb->user->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-12 mb-3">
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <input type="number" class="form-control" id="editJumlahBulanCicilan"
                                    name="jumlah_bulan_cicilan" required>
                            </div>

                            <div class="col-12 mb-3">
                                <label for="editHargaCicilanPerbulan" class="form-label">Harga Cicilan per Bulan</label>
                                <input type="number" class="form-control" id="editHargaCicilanPerbulan"
                                    name="harga_cicilan_perbulan" required>
                            </div>

                            <div class="col-12 mb-3">
                                <label for="newPjb" class="form-label">PJB</label>
                                <input type="file" class="form-control" id="newPjb" name="pjb"
                                    accept=".pdf">
                            </div>
        ...
        <!-- Modal Pembatalan  -->
        <div class="modal fade" id="pembatalanModal" tabindex="-1" aria-labelledby="pembatalanModalLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <form id="formEditPembelian" method="POST" action="{{ route('batal.pembelian') }}">
                        ...
                        <div class="modal-body px-sm-5 mx-50 pb-5">
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <input type="text" class="form-control" id="alasanPembatalan"
                                    name="alasan_pembatalan" required>
                            </div>

                            <div class="col-12 mb-3">
                                <label for="editJumlahBulanCicilan" class="form-label">Jumlah Harga Pengembalian</label>
                                <input type="number" class="form-control" id="editJumlahBulanCicilan"
                                    name="jumlah_pengembalian" required>
                            </div>
